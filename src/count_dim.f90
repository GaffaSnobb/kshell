...
program count_dim
  ...
  use model_space, only: read_sps, set_n_ferm, n_morb_pn, &
       n_j_orbitals_pn, n_j_orbitals, n_ferm, skip_comment, j_orbitalsn, j_orbitals
  implicit none
  ...
  n = maxval(n_id_pn)
  j = maxval(n_j_orbitals)
  allocate( nocc(j, n, 2) )

  do ipn = 1, 2
     do i = 1, n_id_pn(ipn)
        read(lunptn, *) id, nocc(:n_j_orbitals(ipn), i, ipn)
  ...
  do ipn = 1, 2
     do i = 1, n_id_pn(ipn)
        mm = max_m_nocc(nocc(:n_j_orbitals(ipn), i, ipn), ipn)
        if ( maxm_pn(ipn) < mm ) maxm_pn(ipn) = mm
     end do
  end do

  maxm = sum(maxm_pn)
  maxsj = maxval(j_orbitals)
  n = 0 
  do i = 1, maxsj, 2
     n = n + i
  end do
...
contains
  ...
  subroutine gen_ndim_mat( )
    integer :: ipn, id, i, mm
    integer(kdim) :: nd
    integer ::  mlist(maxval(n_j_orbitals)), mlist_max(maxval(n_j_orbitals))
    ...
    do ipn = 1, 2
       ...
       do id = 1, n_id_pn(ipn)
          ...
          do i = 1, n_j_orbitals(ipn)  ! initialize mlist
             mlist_max(i) = maxm_sj(j_orbitalsn(i,ipn), nocc(i ,id, ipn))
          end do
          mlist(:n_j_orbitals(ipn)) = -mlist_max(:n_j_orbitals(ipn))

          mlistloop: do while (.true.)
             nd = 1
             do i = 1, n_j_orbitals(ipn)
                nd = nd * ndim_sj(mlist(i), j_orbitalsn(i, ipn), nocc(i, id, ipn))
             end do
             mm = sum(mlist(:n_j_orbitals(ipn)))
             ndim_mat(mm, id, ipn) = ndim_mat(mm, id, ipn) + nd

             do i = 1, n_j_orbitals(ipn) ! increment mlist
                if (mlist(i) == mlist_max(i)) then 
                   if (i == n_j_orbitals(ipn)) exit mlistloop ! finish mlist
                   cycle 
                end if
                mlist(i) = mlist(i) + 2
